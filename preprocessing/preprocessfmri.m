...
% subjid can be letters and numbers
...
session           = subjectlist{3}{subject_i};
...
pipelinefamily = {'swar', 'swavr', 'swgcar', 'swgcavr', ...
  ...
  'swaor', 'swgcaor', 'swfaor', 'swgcfaor','swcfaor','swcr','sw'};
     ...
     if ismember('g',wholepipeline)
         ...
         if isempty(listfile_file) 
             if strcmp(SPGRfilename,'spgr') 
              ...
              unix(sprintf('cp -Lf %s %s', fullfile(SPGRdir_raw,'spgr.nii*'),SPGRdir));
              unix(sprintf('gunzip -fq %s', fullfile(SPGRdir, [SPGRfilename, '*.gz'])));
 ...
 %----- update list, check input spgr file again ----- 
  ...
  for irun = 1:numrun
    ...
    if isempty(inputimgprefix)
      ...
      unix(sprintf('cp -aLf %s %s', fullfile(unnorm_dir, ['I*', data_type, '*']), ...
        temp_dir));
...
%               fclose(fid);
      ...
      %%%%%%---------------------------------------------------         
        ...
        coreg_func=dir(fullfile(temp_dir, ['c',prevprefix, 'I.nii*']));

        spm_check_registration(SPGRfile_file, [fullfile(temp_dir, coreg_func(1).name),',1'])
        saveas(gcf, [output_log filesep 'check_coreg.png'])
